		...
		try (BufferedReader br = new BufferedReader(new FileReader("shapes.txt"))) {
			...
			//your code
					...
					if (tokens[0].equals("Circle") && tokens.length == 2) {
						...
						if (cirParam > 0) {
							Circle cirObj = new Circle(cirParam);
...
//							cirObj.setRadius(cirParam);
						...
						if (squParam > 0) {
							Square squObj = new Square(squParam);
							//shapes.add(squObj);
...
//							squObj.setWidth(squParam);
						...
						if (recParam1 > 0 && recParam2 > 0) {
							Rectangle recObj = new Rectangle(recParam1, recParam2);
							//shapes.add(recObj);
...
//			System.out.println("test");
		...
		System.out.println("Total Perimeter of Parallelogram is: " + Math.round(totalPerimeter(shapes, count, Parallelogram.class) * 100.0) / 100.0);
